...
const Index = () => {
  ...
  const shouldUpdatePageNumber = (number) => {
    if (
      number > 0 &&
      number <= paltas?.metadata?.totalPages &&
      number != currentPage
    ) {
  ...
  //*PALTAS - swr
  const { data: paltas, error, mutate } = useSWR(
    `api/palta?action=findAll&page=${currentPage}`,
    fetchPaltas
  );

  //*PERMISOS - fetcher de swr
  const fetchSessionInfo = async (url) => {
    const res = await fetch(url);
    const resjson = await res.json();
    return resjson;
  };
  ...
  const handleEditarPalta = (id) => {
    setPaltaEnElModal(paltas.data.filter((it) => it.id === id)[0]);
    ...
    return (
      <div className="flex flex-col h-full justify-center">
        ...
        <div className="space-y-2">
          ...
          <PaginateNavbar
            currentPage={paltas?.metadata?.page}
            totalPages={paltas?.metadata?.totalPages}
